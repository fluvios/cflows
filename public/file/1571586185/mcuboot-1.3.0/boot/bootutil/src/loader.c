 ...
 */
static int boot_status_source( void ) {
 ...
 */
static int boot_previous_swap_type( void ) {
  int post_swap_type;
...
static int boot_read_image_size( int slot, struct image_header *hdr, uint32_t *size ) {
...
static int boot_read_image_header( int slot, struct image_header *out_hdr ) {
  const struct flash_area *fap;
  int area_id;
  int rc;
...
static int boot_read_image_headers( bool require_all ) {
  int rc;
  int i;
...
static uint8_t boot_write_sz( void ) {
  uint8_t elem_sz;
  uint8_t align;
 ...
 * presumably!).
 */
static int boot_slots_compatible( void ) {
 ...
 */
static int boot_read_sectors( void ) {
  int rc;
...
static uint32_t boot_status_internal_off( int idx, int state, int elem_sz ) {
  int idx_sz;
 ...
 * operation.
 */
static int boot_read_status_bytes( const struct flash_area *fap, struct boot_status *bs ) {
  uint32_t off;
  uint8_t status;
  int max_entries;
  int found;
 ...
 */
static int boot_read_status( struct boot_status *bs ) {
  const struct flash_area *fap;
  int status_loc;
  int area_id;
  int rc;
 ...
 */
static int boot_image_check( struct image_header *hdr, const struct flash_area *fap,
...
static int split_image_check( struct image_header *app_hdr,
...
static int boot_check_header_erased( int slot ) {
  const struct flash_area *fap;
...
static int boot_validate_slot( int slot, struct boot_status *bs ) {
  const struct flash_area *fap;
  struct image_header *hdr;
  int rc;
 ...
 */
static int boot_validated_swap_type( struct boot_status *bs ) {
  int swap_type;
...
static uint32_t boot_copy_sz( int last_sector_idx, int *out_first_sector_idx ) {
  size_t scratch_sz;
  uint32_t new_sz;
  uint32_t sz;
  int i;
 ...
 * @return                      0 on success; nonzero on failure.
 */
static inline int boot_erase_sector( const struct flash_area *fap, uint32_t off, uint32_t sz ) {
 ...
 * @return                      0 on success; nonzero on failure.
 */
static int boot_copy_sector( const struct flash_area *fap_src,
...
#ifndef MCUBOOT_OVERWRITE_ONLY
static inline int boot_status_init( const struct flash_area *fap, const struct boot_status *bs ) {
...
#ifndef MCUBOOT_OVERWRITE_ONLY
static int boot_erase_trailer_sectors( const struct flash_area *fap ) {
  uint8_t slot;
  uint32_t sector;
  uint32_t trailer_sz;
  uint32_t total_sz;
 ...
 */
#ifndef MCUBOOT_OVERWRITE_ONLY
static void boot_swap_sectors( int idx, uint32_t sz, struct boot_status *bs ) {
...
static int boot_copy_image( struct boot_status *bs ) {
  size_t sect_count;
  size_t sect;
  int rc;
  size_t size;
...
static int boot_swap_image( struct boot_status *bs ) {
  uint32_t sz;
 ...
 */
#ifndef MCUBOOT_OVERWRITE_ONLY
static int boot_set_copy_done( void ) {
  const struct flash_area *fap;
  int rc;
 ...
 */
#ifndef MCUBOOT_OVERWRITE_ONLY
static int boot_set_image_ok( void ) {
  const struct flash_area *fap;
 ...
 * @return                      0 on success; nonzero on failure.
 */
static int boot_swap_if_needed( int *out_swap_type ) {
  struct boot_status bs;
  int swap_type;
  int rc;
...
int split_go( int loader_slot, int split_slot, void **entry ) {
