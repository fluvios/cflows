...
static uint32_t boot_test_util_area_write_size( int dst_idx, uint32_t off, uint32_t size ) {
...
void boot_test_util_write_image( const struct image_header *hdr, int slot ) {
  uint32_t image_off;
  uint32_t off;
  uint8_t flash_id;
  uint8_t buf[256];
...
static void boot_test_util_write_swap_state( int flash_area_id,
...
void boot_test_util_verify_flash( const struct image_header *hdr0, int orig_slot_0,
