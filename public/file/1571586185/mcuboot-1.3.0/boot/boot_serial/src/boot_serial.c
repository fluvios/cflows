...
int bs_cbor_writer( struct cbor_encoder_writer *cew, const char *data, int len ) {
 ...
 */
static int u32toa( char *tgt, uint32_t val ) {
  char *dst;
  uint32_t d = 1;
  uint32_t dgt;
  int n = 0;
 ...
 */
static void bs_empty_rsp( char *buf, int len ) {
  cbor_encoder_create_map( &bs_root, &bs_rsp, CborIndefiniteLength );
  cbor_encode_text_stringz( &bs_rsp, "rc" );
 ...
 * before restarting.
 */
static void bs_reset( char *buf, int len ) {
 ...
 */
static int boot_serial_in_dec( char *in, int inlen, char *out, int *out_off, int maxout ) {
  int rc;
  uint16_t crc;
  uint16_t len;
