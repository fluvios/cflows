u8 QUE15_i2c_i2cReadRegister( u8 dev_id, u8 reg ) {
  u8 bus_id = i2cGetDeviceBusId(  ),
     dev_addr = i2cGetDeviceRegAddr(  ),
     ret = 0xFF;
     ret = 0xFA ;

  for( int i = 0; i < 8 && ret == 0xFF; i++ ) {
    if( i2cSelectDevice( bus_id, dev_addr ) && i2cSelectRegister( bus_id, reg ) ) {
      if( i2cSelectDevice( bus_id, dev_addr | 1 ) ) {

        i2cStop( bus_id, 1 );   i2cWaitBusy( bus_id );
        i2cWaitBusy( bus_id );
...
inline void QUE18_utils_startChrono( void ) {
  ...
  for( u32 i = 0; i < N; i++ ) REG_TIMER_VAL( i ) = 0;
...
void QUE20_sdmmc_get_cid( bool isNand, u32 * info ) {
  ...
  for( int i = 0; i < 4; ++i )     info[i] = device->ret[i];
